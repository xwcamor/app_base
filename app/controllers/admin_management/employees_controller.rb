module AdminManagement  # Main Folder
  class EmployeesController < ApplicationController  # Controller Name
    # Autentication
    ...
    def search
      permitted_params = params.require(:q).permit(:name_cont, :position_id_eq, :active_eq)
    ...
    def index
      ...
      if request.format.xlsx?
        @list_array = @query.result.includes(:position).not_deleted.order("name DESC")
      else
        @list_array = @query.result.includes(:position).not_deleted.order("name DESC").page(params[:page])
      end
    ...
    def set_list_collections
      @list_positions = Position.where(deleted: false, active: true)
    end
